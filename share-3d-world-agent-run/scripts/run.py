...
def main() -> int:
    ...
    if sys.version_info < (3, 10):
        ...
        return 2
    needs_hub = len(sys.argv) > 1 and sys.argv[1] in {"build", "validate", "review", "upload"} and "--help" not in sys.argv
    executable = sys.executable
    if needs_hub and any(importlib.util.find_spec(m) is None for m in ("huggingface_hub", "pyarrow", "PIL")):
        ...
        try:
            ...
            probe = subprocess.run([executable, "-c", "import huggingface_hub, pyarrow, PIL"], capture_output=True)
            if probe.returncode:
                # Never surface installer output, which can contain private index credentials.
                subprocess.run([executable, "-m", "pip", "install", "huggingface_hub>=0.34,<2", "pyarrow>=18,<26", "pillow>=11,<13"], check=True, capture_output=True)
        except Exception:
