...
description: Capture Blender and bpy creation traces with a saved blend scene, GLB mesh preview and rendered image, redact and validate them, then submit a Hugging Face dataset PR after confirmation. Use during or after Blender modeling tasks. Reject Three.js-only and non-Blender runs.
---
...
## Blender-only eligibility

Accept only projects whose primary 3D modeling was performed in Blender through `bpy`. Inspect the actual creation code and observable execution evidence. Require `artifacts/create.py`, `artifacts/scene.blend`, `artifacts/model.glb`, and `artifacts/preview.png`. A token bpy import, an imported Three.js mesh, or a Blender conversion of another engine's final model does not qualify. Reject non-Blender runs instead of changing their history or adding a synthetic Blender step. Missing trace portions can be marked partial only when Blender provenance and all required artifacts remain available.

Exclude Three.js/React Three Fiber code, JavaScript/TypeScript projects, browser viewers, dependency folders and archives. Include only Blender creation code, its scene and supporting assets/documentation, the GLB export and render. See [references/blender.md](references/blender.md) for the required export, inspection and native dataset viewer format. Both build and upload enforce these gates; never bypass them for an example.

## Public identity and training content

Name the run with a short task title such as **Sculpted Ceramic Teapot**. Use its descriptive slug for the folder; never publish generated run IDs, source session IDs, contributor details, local account paths, private links, source commit references, or checksum inventories. Private hashes and approval bookkeeping remain outside the dataset. Use the allowlisted public schema and the privacy reference to clean every artifact, filename, trace field, report, README, and PR title. Preserve meaningful geometry, code, materials, lighting, interactions, validation and limitations for downstream training. Review personal references semantically; automated pattern matching alone is not a privacy guarantee.
...
3. **Prepare locally.** Export and inspect the Blender scene, its GLB and PNG using the Blender reference.  Follow [references/agent-operations.md](references/agent-operations.md) yourself. Choose the descriptive title, prepare a private terms file for known personal names/handles outside the bundle, and write the observable transcript using [references/schema.md](references/schema.md), build the bundle, and inspect the redacted copies. Text artifacts are redacted before hashing. Re-execute privacy-edited bpy code locally when necessary to verify it still creates the intended scene; never execute untrusted imported code without inspecting it first. Binary files and archives need your inspection or a sanitized replacement; the script cannot certify their contents. Preserve the user's originals.
4. **Validate and prepare the review.** Use the script's `review` operation to validate, check existing HF authentication and the public target, and obtain the bundle's approval digest. Dependencies are handled by the agent runner. Show the task, actual target and contributor account, artifact names and total size, visible step count, redactions, included user attachments, and any missing data. Show the custom PR title and contribution description returned by `review`; the description covers this task, artifacts, automated checks and capture limits. The commit body includes the standalone GLB link for after merge and the skill GitHub link. Once the PR exists, the helper updates its description to link the GLB on that PR branch, so it is viewable before merge. Explain this automatic link resolution in the review; these platform links stay out of training files. Never use the HF CLI/default upload boilerplate. Provide access to the prepared bundle for review and state that the PR and its files will be public.
